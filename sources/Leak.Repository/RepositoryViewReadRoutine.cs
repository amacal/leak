...
namespace Leak.Data.Store
{
    public class RepositoryViewReadRoutine
    {
        ...
        public RepositoryViewReadRoutine(RepositoryViewCache cache, int piece, int block, FileBuffer buffer, RepositoryViewReadCallback callback)
        {
            this.piece = piece;
            this.block = block;
            this.buffer = buffer;
            ...
            this.entries = cache.Find(piece, block, buffer.Count / cache.BlockSize);
        ...
        public void Execute()
        {
            ...
            entries[0].File.Read(position, data, receiver.OnCompleted);
        }
